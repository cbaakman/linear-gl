...
for %%m in (vec quat matrix plane triangle) do (
    %CXX% %CFLAGS% -I include -c src\%%m.cpp -o obj\%%m.o -fPIC
...
for %%t in (vec quat mat) do (
    echo test: %%t
    %CXX% %CFLAGS% -I include -L lib tests\test_%%t.cpp -lboost_unit_test_framework -l%LIB_NAME% -o bin\test_%%t.exe && bin\test_%%t.exe
)
